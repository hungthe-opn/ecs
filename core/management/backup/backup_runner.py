...
from contrib.database.fields.models import LinkRowField
from contrib.database.table.models import Table
...
class stockcnnBackupRunner:
    """
    A class which can backup or restore the database used by an installation of
     Under the hood it uses `pg_dump` and `pg_restore` commands.
    """
    ...
    def __init__(
        ...
        """
        Constructs a stockcnnBackupRunner.
    ...
    def backup_stockcnn(
        ...
        """
        Backs up an entire Database containing an installation of 
        ...
        :raises InvalidstockcnnBackupArchive: When the provided backup archive does not
            meet the expected stockcnn backup format.
...
def _get_sorted_user_tables_names(conn) -> List[str]:
    """
    Queries the provided connection for the table and schema names of stockcnn's user
...
def _default_backup_location(database):
    ...
    return f"stockcnn_backup_{database}_{now}.tar.gz"


def add_shared_postgres_command_args(parser):
    """
    The DB connection parameters are shared between the backup_stockcnn and
    restore_stockcnn commands.
    """
