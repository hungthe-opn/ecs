...
class Command(BaseCommand):
    help = """
        Backs up a stockcnn database into a single compressed archive which can be
        restored using the restore_stockcnn stockcnn management command.
        ...
        being updated and not connected to a running version of stockcnn for the
        ...
        safe you should only perform back-up's when your stockcnn server is shut down
        or you have first copied the database to a new cluster which is not in active
        use.

        The back-up is split into batches as often stockcnn database's can end up with
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "`stockcnn_backup_{database}_{datetime}.tar.gz`",
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            backup_file_name = runner.backup_stockcnn(file, batch_size, additional_args)
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully backed up stockcnn to {backup_file_name} which can "
                    f"be restored using the ./stockcnn restore_stockcnn command. "
                )
            )
        ...
        except InvalidstockcnnBackupArchive:
            raise CommandError(
                "Please ensure the provided back-up file is a valid "
                "stockcnn backup file produced by ./stockcnn "
                "backup_stockcnn"
            )
